...
mode = 'train'
save_model_name = 'params_1_coco_skeleton_from_keypoints_merge.pkl'
...
class creatModel(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(nStack):
            ...
            out_skeleton = out_keypoints[:, np.array(sks)+1, :, :][:, :, 0, :, :] + out_keypoints[:, np.array(sks)+1, :, :][:, :, 1, :, :]
